...
use Relevate\HttpClient\Cache\CacheInterface;
use Relevate\HttpClient\Cache\FilesystemCache;

/**
 * Performs requests on Relevate API using If-Modified-Since headers.
...
class CachedHttpClient extends HttpClient
{
    ...
    public function __construct(array $options = array(), ClientInterface $client = null, CacheInterface $cache = null)
    {
        ...
        $this->cache = $cache ?: new FilesystemCache($this->options['cache_dir'] ?: sys_get_temp_dir().DIRECTORY_SEPARATOR.'php-Relevate-api-cache');
    }
